...
def main():
    ...
    config = {"configurable": {"thread_id": "deep-research-rag-001"}}
    
    # 可以從輸入獲取，或使用預設問題
    user_input = input("請輸入您的研究問題（或按 Enter 使用預設問題）: ").strip()
    if not user_input:
        user_input = "比較微軟(MSFT)和谷歌(GOOGL)在AI領域的佈局，並結合 Tree of Thoughts 論文中的方法論進行分析"
